...
const writeListVideoId = (listVideoId) => {
  fs.writeFileSync("./bot-youtube-ger/listId.txt", listVideoId, (err) => {
    if (err) {
      console.error(err);
      return;
    }
...
const writeFileJSON = (text) => {
  ...
  const formattedDate = `./bot-youtube-ger/data/${year}-${month}-${day}.json`;
...
const readFileListVideoId = () => {
  return new Promise((resolve, reject) => {
    fs.readFile("./bot-youtube-ger/listId.txt", "utf8", (err, data) => {
      if (err) {
        resolve([]);
        return;
      }
...
const detectLanguage = async (text) => {
  ...
  const result = cld3.getLanguage(text)?.filter((i) => i?.language == 'en'  || i?.language == 'de'   );
