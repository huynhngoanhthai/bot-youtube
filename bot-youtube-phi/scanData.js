...
const writeListVideoId = (listVideoId) => {
  fs.writeFileSync("./bot-youtube-phi/listId.txt", listVideoId, (err) => {
    if (err) {
      console.error(err);
      return;
    }
...
const writeFileJSON = (text) => {
  ...
  const formattedDate = `./bot-youtube-phi/data/${year}-${month}-${day}.json`;
...
const readFileListVideoId = () => {
  return new Promise((resolve, reject) => {
    fs.readFile("./bot-youtube-phi/listId.txt", "utf8", (err, data) => {
      if (err) {
        resolve([]);
        return;
      }
...
const detectLanguage = async (text) => {
  ...
  const result = cld3.getLanguage(text)?.filter((i) => i?.language == 'fil'   || (i?.language == 'en'&& i?.proportion > 0.8)  || (i?.language == 'zh' && i?.proportion > 0.8) );
