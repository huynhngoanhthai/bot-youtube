...
const writeListVideoId = (listVideoId) => {
  fs.writeFileSync("./bot-youtube-aus/listId.txt", listVideoId, (err) => {
    if (err) {
      console.error(err);
      return;
    }
...
const writeFileJSON = (text) => {
  ...
  const formattedDate = `./bot-youtube-aus/data/${year}-${month}-${day}.json`;
...
const readFileListVideoId = () => {
  return new Promise((resolve, reject) => {
    fs.readFile("./bot-youtube-aus/listId.txt", "utf8", (err, data) => {
      if (err) {
        resolve([]);
        return;
      }
