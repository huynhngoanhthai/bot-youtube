...
const writeListVideoId = (listVideoId) => {
  fs.writeFileSync("./bot-youtube-tw/listId.txt", listVideoId, (err) => {
    if (err) {
      console.error(err);
      return;
    }
...
const writeFileJSON = (text) => {
  ...
  const formattedDate = `./bot-youtube-tw/data/${year}-${month}-${day}.json`;
...
const readFileListVideoId = () => {
  return new Promise((resolve, reject) => {
    fs.readFile("./bot-youtube-tw/listId.txt", "utf8", (err, data) => {
      if (err) {
        resolve([]);
        return;
      }
...
const detectLanguage = async (text) => {
  ...
  const result = cld3.getLanguage(text)?.filter((i) => i?.language == 'zh' || i?.language == 'zh-tw');
    ...
    while (count < contains.SCANS) {
      ...
      if (!listVideoId.includes(videoID)) {
        ...
        if (video.lang) {
          // likeAndSubscribe
          if (count < 50) {
            await page.goto(videoURL + contains.SERVER);
            await page.waitForSelector("#thumbnail");
            await page.waitForTimeout(1000);
            // use click like
            await page.evaluate(() => {
              const toggleButton = document.querySelector('ytd-toggle-button-renderer#like-button');
              if (toggleButton) {
